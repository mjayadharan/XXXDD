...
namespace Projector
{
...

  {
    ...
    for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell;
      ...
      // interested in
      if (boundary_functions.find(cell->face(face)->
          ...
          if (fe_is_system)
            // FE has several components
            {
              boundary_functions
                ...
                ->second->vector_value_list(fe_values.
              ...
              if (coefficient_is_vector)
                ...
                // components
                coefficient->vector_value_list(fe_values.
              ...
              else
                {
                  ...
                  if (coefficient != nullptr)
                    coefficient->value_list(fe_values.
              ...
              // Same approach does not work for Hcurl, so we throw an
              // exception. Default value 1.0 allows for use with non Hdiv
              // elements
    ...
    Vector<number> &                            rhs_vector)
  {
    ...
    for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell;
         ++face)
      {
        ...
        if (boundary_functions.find(copy_data.cell->face(face)->
    ...
    WorkStream::run(
      ...
      static_cast<std::function<
        void(typename DoFHandler<dim, spacedim>::active_cell_iterator const &,
             Scratch const &,
             CopyData<DoFHandler<dim, spacedim>, number> &)>>(
        std::bind(&create_boundary_mass_matrix_1<dim, spacedim, number>,
                  std::placeholders::_1,
                  std::placeholders::_2,
                  std::placeholders::_3,
                  std::cref(mapping),
      ...
      static_cast<std::function<void(
        ...
        std::bind(&copy_boundary_mass_matrix_1<dim, spacedim, number>,
                  std::placeholders::_1,
                  std::cref(boundary_functions),
  ...
  template <typename number>
  void
  Projector<dim>::invert_mass_matrix(const SparseMatrix<number> &mass_matrix,
                                     const Vector<number> &      rhs,
                                     Vector<number> &            solution)
  {
    //      ReductionControl      control(5*rhs.size(), 0., 1e-12, false,
    //      false); GrowingVectorMemory<Vector<number> > memory;
    ...
    std::vector<unsigned int>                  component_mapping)
  {
    ...
    if (component_mapping.size() == 0)
      ...
      Assert(dof.get_fe(0).n_components() == component_mapping.size(),
             ExcInternalError());

    if (state == false)
      {
        for (typename std::map<types::boundary_id,
                               const Function<dim, double> *>::const_iterator
    ...
    create_boundary_mass_matrix(mapping,
                                dof,
                                q,
                                mass_matrix,
                                boundary_functions,
    ...
    std::vector<unsigned int>  component_mapping)
  {
    ...
    do_project_boundary_values(
      dof, boundary_functions, q, boundary_values, component_mapping);
